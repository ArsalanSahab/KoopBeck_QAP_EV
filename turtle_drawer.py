...
class TurtleDrawer(turtle.Turtle):
    ...
    def __init__(self):
        ...
        self.speed(TurtleDrawer.DRAWING_SPEED)
        if not TurtleDrawer.SHOW_TURTLE:
            self.hideturtle()
    ...
    def draw_points(self, best_chromosome):
        ...
        circle_radius = TurtleDrawer.DISTANCE_BETWEEN_POINTS / (2 * (math.sin(math.pi / number_of_points)))
        ...
        for draw_index in range(number_of_points):
            self.circle(TurtleDrawer.POINT_RADIUS)
            self.coordinates_list[draw_index] = self.pos()
            self.penup()
            self.left(90)
            self.forward(20)
            self.write(best_chromosome[draw_index], font=TurtleDrawer.FONT_CONFIG)
            self.backward(20)
            self.right(90)
            self.right((360 / number_of_points))
            self.forward(TurtleDrawer.DISTANCE_BETWEEN_POINTS)
            self.pendown()
    ...
    def __draw_generation_info(self, generation_number, best_score, best_chromosome):
        ...
        self.penup()
        self.goto(-self.screen.window_width() / 2 + TurtleDrawer.CAPTIONS_MARGIN,
                  self.screen.window_height() / 2 - TurtleDrawer.CAPTIONS_MARGIN)
        self.write(f'Best chromosome: {best_chromosome}', font=TurtleDrawer.FONT_CONFIG)
        self.right(90)
        self.forward(20)
